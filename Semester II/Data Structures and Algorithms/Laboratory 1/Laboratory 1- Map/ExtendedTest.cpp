...
void testGetKeyRange() {
	Map m;
	cout << "Test extra functionality given at the lab - function getKeyRange " << endl;
	assert(m.getKeyRange() == -1); // we expect -1
	m.add(5, 5);
	m.add(4, 5);
	assert(m.getKeyRange() == 1); // we expect 1
	m.add(8, 5);
	...
	m.add(10, 5);
	m.add(9, 3);
	assert(m.getKeyRange() == 16); //we expect 16, because the max key is 20 and the min is 4
	m.add(2, 5);
	assert(m.getKeyRange() == 18); //we expect 18
	m.remove(20);
	assert(m.getKeyRange() == 8); // we expect 8
